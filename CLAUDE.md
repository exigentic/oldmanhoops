...
Two scheduled routes: `housekeeping` (07:00 UTC = 2am EST / 3am EDT) and `remind` (fires at both 12:00 and 13:00 UTC, but the handler exits early unless the local hour matches `REMINDER_LOCAL_HOUR` — stable 8am ET year-round). Both gated by `lib/cron-auth.ts` using the `CRON_SECRET` Bearer token. Failures call `notifyAdmin()` to email `ADMIN_EMAIL`.
